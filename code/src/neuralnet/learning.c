...
static full_output_t* allocate_full_output (const neuralnet_t* net)
{
    ...
    {
        full_output->values[layer] =
            SAFE_MALLOC (net->neurons_per_layer[layer] * sizeof (float));
    }
...
static full_output_t* calculate_full_output (const neuralnet_t* net,
                                             const float* input)
{
    ...
    for (size_t gap = 0; gap < net->layer_count - 1; ++gap)
    {
        ...
        {
            fout->values[gap + 1][to] = net->edges[gap][net->neurons_per_layer[gap]][to];

            for (size_t from = 0; from < net->neurons_per_layer[gap]; ++from)
            {
                fout->values[gap + 1][to] +=
                    fout->values[gap][from] * net->edges[gap][from][to];
            }
                         ...
                         const float* target_output)
{
    ...
    float* ins = sigmoidize (input, net->neurons_per_layer[0]);
    float* touts = sigmoidize (target_output,
    ...
    size_t last = net->layer_count - 1;

    errors[last] = SAFE_MALLOC (net->neurons_per_layer[last] * sizeof (float));

    for (size_t to = 0; to < net->neurons_per_layer[last]; ++to)
    {
        errors[last][to] =
            calculate_sigmoid_error (touts[to], fout->values[last][to]);

        // Correcting edge weights of gap right before the last layer of neurons
        for (size_t from = 0; from < net->neurons_per_layer[last - 1]; ++from)
        {
            net->edges[last - 1][from][to] +=
    ...
    for (size_t layer = net->layer_count - 2; layer > 0; --layer)
    {
        errors[layer] =
            SAFE_MALLOC (net->neurons_per_layer[layer] * sizeof (float));

        for (size_t current = 0; current < net->neurons_per_layer[layer];
             ++current)
        {
            errors[layer][current] = 0.0f;

            for (size_t right = 0; right < net->neurons_per_layer[layer + 1];
                 ++right)
            {
                errors[layer][current] += errors[layer + 1][right] *
                                          net->edges[layer][current][right];
            }

            errors[layer][current] *=
                calculate_sigmoid_error_term (fout->values[layer][current]);

            // Correcting edge weights of gap right before the present layer of
            // neurons
            for (size_t left = 0; left < net->neurons_per_layer[layer - 1];
                 ++left)
            {
                net->edges[layer - 1][left][current] +=
    ...
    free (ins);
    free (touts);
    for (size_t layer = 1; layer < net->layer_count; ++layer)
        free (errors[layer]);
    free (errors);
}
