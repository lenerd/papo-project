...
void backpropagate(neuralnet_t* net, const float* input, const float* target_output){
	...
	for(size_t layer = net->layer_count - 2; layer > 0; --layer){
		...
		for(size_t current = 0; current < net->neurons_per_layer[layer]; ++current){

			errors[layer][current] = 0.0f;

			for(size_t right = 0; right < net->neurons_per_layer[layer + 1]; ++right){
			...
			for(size_t left = 0; left < net->neurons_per_layer[layer - 1]; ++left){
